...
def classify_coverage_v2(play_data):
    ...
    DEEP_SAFETY_MIN = 5 
    DEEP_CORNER_MIN = 5    # ~5.7 mapped yards
    PRESS_YARDS = 3         # ~1.3 mapped yards
    MAN_MATCH_YARDS = 12    # ~3.2 mapped yards
    SHALLOW_FLAT_YARDS = 4.0  # ~2.5 mapped yards
    ALIGNMENT_Y_DIFF = 1.5           # 1 yard difference in y_yards → man indicator
    ...
    deep_corners = [c for c in corners if abs(c['coordinates']['x_yards']) >= DEEP_CORNER_MIN]
    ...
    if receivers and dbs:
        available_receivers = list(receivers)  # Create a mutable copy of receivers
        
        # Sort DBs by y-coordinates (left to right)
        sorted_dbs = sorted(dbs, key=lambda db: db['coordinates']['y_yards'])
        ...
        for db in sorted_dbs:
            ...
            closest_receiver = min(available_receivers, 
                                 key=lambda r: abs(r['coordinates']['y_yards'] - db['coordinates']['y_yards']))
            y_diff = abs(db['coordinates']['y_yards'] - closest_receiver['coordinates']['y_yards'])
            distance = calculate_distance(db, closest_receiver)

            # Proximity indicator
            if distance <= MAN_MATCH_YARDS and y_diff <= ALIGNMENT_Y_DIFF:
                man_signals += 1
                print(f"MAN SIGNAL: DB ({db.get('position')}) at (x:{db['coordinates']['x_yards']:.1f}, y:{db['coordinates']['y_yards']:.1f}) vs " 
                      f"Receiver ({closest_receiver.get('position')}) at (x:{closest_receiver['coordinates']['x_yards']:.1f}, y:{closest_receiver['coordinates']['y_yards']:.1f})"
                      f"\nY-Align: {y_diff:.2f}, Total Dist: {distance:.2f}\n")
            else:
                zone_signals += 1
                print(f"ZONE SIGNAL: DB ({db.get('position')}) at (x:{db['coordinates']['x_yards']:.1f}, y:{db['coordinates']['y_yards']:.1f}) vs "
                      f"Receiver ({closest_receiver.get('position')}) at (x:{closest_receiver['coordinates']['x_yards']:.1f}, y:{closest_receiver['coordinates']['y_yards']:.1f})"
                      f"\nY-Align: {y_diff:.2f}, Total Dist: {distance:.2f}\n")

            available_receivers.remove(closest_receiver)
