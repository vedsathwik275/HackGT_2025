...
classifyCoverageV2(playData) {
  ...
  const DEEP_CORNER_MIN = 5;    // ~5.7 mapped yards (kept for backward compatibility if needed)
  const PRESS_YARDS = 3;        // ~1.3 mapped yards
  const MAN_MATCH_YARDS = 12;   // distance threshold used when selecting closest DB (kept for safety)
  const SHALLOW_FLAT_YARDS = 4.0;
  const ALIGNMENT_Y_DIFF = 1;   // y difference (yards) to consider alignment -> man
  ...
  // When matched, remove that DB from availableDbs and increment manSignals.
  for (const receiver of receivers) {
    // find aligned DB candidates among remaining DBs
    const candidates = availableDbs.filter(db => 
      Math.abs(db.coordinates.yYards - receiver.coordinates.yYards) <= ALIGNMENT_Y_DIFF &&
      Math.abs(db.coordinates.xYards) <= 7
    );
    ...
    const closestDb = candidates.reduce((closest, db) => {
      const currentDistance = this.calculateDistance(db, receiver);
      const closestDistance = this.calculateDistance(closest, receiver);
      return currentDistance < closestDistance ? db : closest;
    });
  ...
  for (const db of availableDbs) {
    const distFromLos = Math.abs(db.coordinates.xYards);
  ...
  const avgLbDepth = lbDepths.length > 0 ? 
    Math.abs(lbDepths.reduce((sum, depth) => sum + depth, 0)) / lbDepths.length : 0;
  ...
  const coverageAnalysis = {
    ...
    analysis: {
      ...
      deep_corners: dbs.filter(c => Math.abs(c.coordinates.xYards) >= DEEP_CORNER_MIN).length,
