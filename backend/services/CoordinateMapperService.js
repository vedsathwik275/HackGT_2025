...
class CoordinateMapperService {
  ...
  mapCoordinates(detectionData, lineOfScrimmageX, fieldDims) {
    ...
    const { offense, defense, special } = this.classifyOffenseDefense(players, lineOfScrimmageX);
      ...
      if (this.isPlayer(detection)) {
        ...
        // Build player payload and also compute an inferred position based on mapped coordinates
        const playerData = {
          detectionId: detection.detection_id,
          position: detection.class, // original model label (kept for auditing)
          team: team,
          coordinates: {
        ...
        // Add an inferred position computed from coordinates; does not overwrite model label
        try {
          playerData.inferredPosition = this.inferPositionFromCoordinates(playerData);
        } catch (err) {
          // Non-fatal: if inference fails, leave inferredPosition undefined
          playerData.inferredPosition = undefined;
        }
  ...
  /**
   * Infer a football position from mapped coordinates and team context.
   * This is a lightweight heuristic to allow downstream overrides.
   * Returns a short position string (e.g., 'QB','WR','RB','LB','CB','S','OL') or null.
   */
  inferPositionFromCoordinates(playerData) {
    if (!playerData || !playerData.coordinates) return null;

    const { xYards, yYards } = playerData.coordinates;
    const team = playerData.team || 'unknown';

    // Heuristic rules (simple and conservative):
    // - QB: near center (y close to 0) and close to LOS (x around 0) and typically in backfield (small x magnitude)
    // - RB: near backfield and slightly off-center
    // - WR: large absolute y (toward sidelines)
    // - OL/OG/OT: very close to LOS, small x and small |y|
    // - LB/CB/S: defensive backs/linebackers are usually across the LOS and range of y

    const absX = Math.abs(xYards);
    const absY = Math.abs(yYards);

    // QB
    if (absX <= 3 && absY <= 5) return 'QB';

    // Offensive line (very close to LOS horizontally and near center vertically)
    if (absX <= 1.5 && absY <= 6 && team === 'offense') return 'OL';

    // Running back - behind QB but still near center
    if (absX > 2 && absX <= 8 && absY <= 6 && team === 'offense') return 'RB';

    // Wide receiver - far toward sideline
    if (absY > 8 && team === 'offense') return 'WR';

    // Defensive back / corner - far toward sideline on defense
    if (absY > 8 && team === 'defense') return 'CB';

    // Linebacker - middle of field on defense
    if (absY <= 8 && absX <= 12 && team === 'defense') return 'LB';

    // Fallback: if close to center vertical, likely receiver/slot or safety
    if (absY <= 6 && absX > 8 && team === 'defense') return 'S';
